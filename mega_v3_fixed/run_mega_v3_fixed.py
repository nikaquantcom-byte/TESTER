...
FIXES vs original run_mega_v3.py:
  - Phase 4 now imports from walk_forward_v3 (WFConfig with n_splits/train_pct/min_trades)
  - run_walk_forward signature corrected (eng, conf, tp, cfg)
  - WF result dict keys corrected (is_results / oos_results)

Usage:
    python -m mega_v3_fixed --data "XAUUSD_M10.csv" --top 50
    python -m mega_v3_fixed --data "XAUUSD_M10.csv" --top 50 --skip-wf
...
# ── FIXED: import from walk_forward_v3 ──
...
def main():
    ...
    # ── Phase 4: Walk-Forward Validation (FIXED) ──
    wf_results = []
    if not args.skip_wf:
        if p3:
        ...
        # ── FIXED: WFConfig now uses n_splits / train_pct / min_trades ──
        wf_cfg = WFConfig(
        ...
        for rank, candidate in enumerate(wf_candidates):
            ...
            try:
                ...
                # ── FIXED: result dict keys are is_results / oos_results ──
                ...
                print(f"    Summary: {wf['summary']['robustness']}")
    ...
    if wf_results and any('oos_pf' in w and w.get('robustness', 0) > 0.6 for w in wf_results):
        ...
        if robust:
            ...
            best_source = f"Phase 4 WF (OOS PF={best['oos_pf']:.2f}, Robust={best['robustness']:.2f})"
    elif p3:
    ...
    from nika_optimizer.backtest_engine_v2 import run_backtest
    n = len(ohlcv['close'])
