"""
MEGA Optimizer V3 — Universal. Test EVERYTHING.
...
Usage:
    python -m nika_optimizer --data "XAUUSD_M10.csv"           # full sweep
    python -m nika_optimizer --data "XAUUSD_M10.csv" --quick   # fast test
"""
...
def main():
    parser = argparse.ArgumentParser(description='NikaQuant MEGA Optimizer V3 — Test EVERYTHING')
    ...
    print("\n" + "=" * 80)
    print("  NikaQuant MEGA Optimizer V3 — Test EVERYTHING")
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # PHASE 1 — Signal Tournament (958K combos)
    # Every trigger type, every MA, every mode, every param — NO ASSUMPTIONS
    # ══════════════════════════════════════════════════════════════════════════
    ...
    # Top engines × 255 indicator masks × min_agree levels
    # ══════════════════════════════════════════════════════════════════════════
    top_engines = [eng for eng, res, score in p1[:args.top] if score > 0]
    if not top_engines:
        top_engines = [p1[0][0]]

    print(f"\n[Phase 2] Taking top {len(top_engines)} engines into confluence sweep...")
    ...
    # Top Phase-2 configs × BE × Trail × Hard Stop × PSAR exit × Spread
    # ══════════════════════════════════════════════════════════════════════════
    p3 = []
    if not args.skip_phase3 and len(p2) > 0:
        ...
        print(f"\n[Phase 3] Taking top {len(top_phase2)} Phase-2 configs into trade management sweep...")
    ...
    # Top configs → rolling window OOS test to detect overfitting
    # ══════════════════════════════════════════════════════════════════════════
    wf_results = []
    if not args.skip_wf:
        # Pick best configs from Phase 3 (or Phase 2 if Phase 3 skipped)
        if p3:
            wf_candidates = p3[:args.wf_top]
        else:
            wf_candidates = [(eng, conf, make_trade_params(spread_points=0.30, use_open_entry=1.0), score, res)
                             for eng, conf, res, score in p2[:args.wf_top]]

        print(f"\n[Phase 4] Walk-Forward Validation on top {len(wf_candidates)} configs...")
        print(f"  Window: 70% train / 30% test | Rolling 5 folds")
        ...
        for rank, candidate in enumerate(wf_candidates):
            ...
            try:
                wf = run_walk_forward(
                    ohlcv, eng, conf, tp, wf_cfg,
                )
                oos_pf = np.mean([r[R_PROFIT_FACTOR] for r in wf['oos_results']])
                oos_wr = np.mean([r[R_WIN_RATE] for r in wf['oos_results']])
                oos_trades = sum([r[R_TOTAL_TRADES] for r in wf['oos_results']])
                is_pf = np.mean([r[R_PROFIT_FACTOR] for r in wf['is_results']])

                robustness = oos_pf / max(is_pf, 0.01)
                status = "✅ ROBUST" if robustness > 0.60 and oos_pf > 1.0 else "⚠️ FRAGILE" if oos_pf > 1.0 else "❌ OVERFIT"

                print(f"    IS  PF: {is_pf:.2f}")
                print(f"    OOS PF: {oos_pf:.2f} | WR: {oos_wr:.1f}% | Trades: {int(oos_trades)}")
        ...
        print(f"{'='*100}")
        print(f"  {'#':>3} | {'Trigger':>12} | {'IS PF':>6} | {'OOS PF':>6} | {'OOS WR':>6} | {'OOS Trades':>10} | {'Robust':>6} | Status")
        print(f"  {'-'*100}")
        for wfr in wf_results:
            if 'oos_pf' in wfr:
                print(f"  {wfr['rank']:>3} | {wfr['trigger']:>12} | {wfr['is_pf']:6.2f} | {wfr['oos_pf']:6.2f} | "
                      f"{wfr['oos_wr']:5.1f}% | {wfr['oos_trades']:>10} | {wfr['robustness']:6.2f} | {wfr['status']}")
            else:
                print(f"  {wfr['rank']:>3} | {wfr['trigger']:>12} | {wfr['status']}")
        print(f"{'='*100}")
    else:
        t_p4 = time.time()
        print("\n[Phase 4] Walk-Forward skipped.")

    # ══════════════════════════════════════════════════════════════════════════
    # FINAL OUTPUT — Best config + TradingView settings
    # ══════════════════════════════════════════════════════════════════════════

    # Pick the best robust config (from WF if available, else Phase 3, else Phase 2)
    best_source = "Phase 2"
    if wf_results and any('oos_pf' in w and w.get('robustness', 0) > 0.6 for w in wf_results):
        ...
        if robust:
            ...
            best_tp = best.get('trade_params', make_trade_params(spread_points=0.30, use_open_entry=1.0))
            best_res_oos = best.get('oos_pf', 0)
            best_source = f"Phase 4 WF (OOS PF={best_res_oos:.2f}, Robust={best['robustness']:.2f})"
    elif p3:
        best_eng, best_conf, best_tp, _, best_res_raw = p3[0]
        best_source = "Phase 3"
    elif p2:
        best_eng, best_conf, best_res_raw, _ = p2[0]
        best_tp = make_trade_params(spread_points=0.30, use_open_entry=1.0)
        best_source = "Phase 2"
    else:
        best_eng, best_res_raw, _ = p1[0]
        best_conf = ConfluenceConfig()
        best_tp = make_trade_params(spread_points=0.30, use_open_entry=1.0)
        best_source = "Phase 1"

    # Get full results for best config
    ...
    from .backtest_engine_v2 import run_backtest
    n = len(ohlcv['close'])
    ...
    tv_settings = {
        ...
        'linreg_length': best_eng.linreg_length,
        'linreg_ma_type': MA_NAMES[best_eng.linreg_ma_type],
        'linreg_ma_period': best_eng.linreg_ma_period,
        'confluence': {
            'active_indicators': [IND_NAMES[j] for j in range(NUM_IND) if best_conf.indicator_mask & (1 << j)],
            'min_agree': best_conf.min_agree,
            'mtf_block': best_conf.mtf_block,
        },
        'trade_management': {
    ...
    engine_json = f'{output_dir}/engine_config.json'
    with open(engine_json, 'w') as f:
        json.dump(dict(best_eng._asdict()), f, indent=2)
    print(f"  📄 Saved: {engine_json}")
    ...
    print(f"\n  🚀 MEGA V3 COMPLETE")
    print(f"  Paste best_params.json into your TradingView Pine Script V7 Pro indicator.")
    print(f"  DATA IS KING. 👑\n")
