...
from .t3_engine import (
    _ema, _rma, _t3, atr_calc, rsi_calc, tsi_val, adx_di,
    compute_t3_signals, psar_calc if False else None,
    STRUCT_OF_IND,
)
...
class ConfluenceConfig(NamedTuple):
    ...
    di_diff:        int     # 0=OFF 1=ON (DI+ > DI- for longs)
    vol_filter:     int     # 0=OFF 1=ON (volume > SMA20)
    n_of_m:         int     # how many active filters must agree (1,2,3)


ADX_THRESHOLDS  = [0.0, 20.0, 25.0, 30.0]   # 0 = OFF
RSI_ZONES       = [0, 1, 2]                  # 0=OFF 1=30/70 2=40/60
TSI_DIRS        = [0, 1]
PSAR_AGREES     = [0, 1]
DI_DIFFS        = [0, 1]
VOL_FILTERS     = [0, 1]
                        ...
                        for di in DI_DIFFS:
                            for vol in VOL_FILTERS:
                                ...
                                for nom in N_OF_M_VALUES:
                                    if nom > max(n_active, 1): continue
...
def _apply_confluence(buy_raw, sell_raw, cfg, d):
    """Apply confluence filters to raw T3 signals. Returns filtered buy/sell."""
    n = len(buy_raw)
    buy  = buy_raw.copy()
    sell = sell_raw.copy()
    ...
    for i in range(n):
        ...
        if cfg.rsi_zone > 0:
            ...
            if rsi_v[i] < hi:  votes_buy  += 1
            if rsi_v[i] > lo:  votes_sell += 1

        # TSI direction
        if cfg.tsi_dir > 0:
            n_active += 1
            if tsi_v[i] > 0:  votes_buy  += 1
            if tsi_v[i] < 0:  votes_sell += 1

        # PSAR agreement
        if cfg.psar_agree > 0:
            n_active += 1
            if psar_b[i]:      votes_buy  += 1
            if not psar_b[i]:  votes_sell += 1

        # DI+ > DI-
        if cfg.di_diff > 0:
            n_active += 1
            if dip_v[i] > dim_v[i]: votes_buy  += 1
            if dim_v[i] > dip_v[i]: votes_sell += 1

        # Volume filter
        if cfg.vol_filter > 0:
            n_active += 1
            if vol_v[i] > vol_sma[i]:
                votes_buy  += 1
                votes_sell += 1

        # N-of-M gate
        if n_active > 0:
            if buy[i]  and votes_buy  < cfg.n_of_m: buy[i]  = False
            if sell[i] and votes_sell < cfg.n_of_m: sell[i] = False

    return buy, sell


@njit_safe
def _apply_confluence_njit(buy_raw, sell_raw,
                            adx_v, dip_v, dim_v, rsi_v, tsi_v,
                            psar_bull, vol_v, vol_sma,
                            adx_thresh, rsi_zone, tsi_dir,
                            psar_agree, di_diff, vol_filter, n_of_m):
    """Numba-compiled confluence filter."""
    ...
    for i in range(n):
        if not buy[i] and not sell[i]:
            continue
        votes_buy = 0; votes_sell = 0; n_active = 0
        if adx_thresh > 0.0:
            n_active += 1
            if adx_v[i] >= adx_thresh: votes_buy += 1; votes_sell += 1
        if rsi_zone > 0:
            n_active += 1
            lo = 30.0 if rsi_zone == 1 else 40.0
            hi = 70.0 if rsi_zone == 1 else 60.0
            if rsi_v[i] < hi: votes_buy  += 1
            if rsi_v[i] > lo: votes_sell += 1
        if tsi_dir > 0:
            n_active += 1
            if tsi_v[i] > 0.0: votes_buy  += 1
            if tsi_v[i] < 0.0: votes_sell += 1
        if psar_agree > 0:
            n_active += 1
            if psar_bull[i]:      votes_buy  += 1
            if not psar_bull[i]:  votes_sell += 1
        if di_diff > 0:
            n_active += 1
            if dip_v[i] > dim_v[i]: votes_buy  += 1
            if dim_v[i] > dip_v[i]: votes_sell += 1
        if vol_filter > 0:
            n_active += 1
            if vol_v[i] > vol_sma[i]: votes_buy += 1; votes_sell += 1
        if n_active > 0:
            if buy[i]  and votes_buy  < n_of_m: buy[i]  = False
            if sell[i] and votes_sell < n_of_m: sell[i] = False
    return buy, sell


def njit_safe(fn):
    """Decorator: compile with njit, fall back to pure Python on failure."""
    try:
        from numba import njit as _njit
        return _njit(_apply_confluence_njit.__wrapped__ if hasattr(fn,'__wrapped__') else fn)
    except Exception:
        return fn
...
def _apply_confluence_fast(buy_raw, sell_raw, cfg,
                            adx_v, dip_v, dim_v, rsi_v, tsi_v,
                            psar_bull, vol_v, vol_sma):
    """Pure-numpy confluence filter (no njit needed at call-time)."""
    from numba import njit
    n = len(buy_raw)
    buy  = buy_raw.copy()
    sell = sell_raw.copy()
